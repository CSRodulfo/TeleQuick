...
export class CustomersComponent implements OnInit, OnDestroy {
  ...
  getFromDisk() {
    return this.localStorage.getDataObject(`${CustomersComponent.DBKeyTodoDemo}:${this.currentUserId}`);
  }

  saveToDisk() {
    if (this.isDataLoaded) {
      this.localStorage.saveSyncedSessionData(this.rowsCache, `${CustomersComponent.DBKeyTodoDemo}:${this.currentUserId}`);
    }
  }
}
